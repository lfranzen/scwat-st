...
cage_ann_data2[cage_ann_data2$hgnc_symbol %in% "DEPP1",]
...
```{r deseq_oble_restable_plot, fig.width=1.5, fig.height=1, fig.show='hold'}
...
# rownames(d_ins) <- gene_table[rownames(d_ins), "gene_name_2"]
...
```{r deseq_ins_restable_plot, fig.width=1.5, fig.height=1, fig.show='hold'}
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 4, height = 4);p;dev.off()
...
glabs_ins <- res_ins_df[res_ins_df$gene_labels != "" & !is.na(res_ins_df$gene_labels), "gene_labels"]
# for(l in glabs_ins){res_ins_df[l, "gene_labels"] <- gene_table[gene_table$gene_name_2 == l, "gene_name"]}
  
# res_ins_df$log2FoldChange <- res_ins_df$log2FoldChange * -1
minmax_xilm <- round( max(abs(res_ins_df$log2FoldChange)) + 1 )
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 10, height = 4.5);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 6, height = 4.5);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 8, height = 8);p;dev.off()
...
# VlnPlot(subset(se_ins, INSR>0), features = c("INSR"), slot = "scale.data", pt.size = 0.01)
# VlnPlot(se_ins, features = c("ADIPOQ", "LEP"), group.by = "cluster_anno", slot = "scale.data", pt.size = 0)
# VlnPlot(se_ins, features = c("ADIPOQ", "LEP"), group.by = "seurat_clusters", slot = "scale.data", pt.size = 0)

se_ins_data_all <- GetAssayData(subset(se_ins, cluster_anno %in% c("Adipocyte 1", "Adipocyte 2")), slot = "data")
```
...
for(s in s_names){
  ...
  for(c in c(4, 5)){
    c_spots <- colnames(subset(se_ins, seurat_clusters == c))
    sc_spots <- intersect(c_spots, s_spots)
    sc_data <- data.frame(Matrix::rowMeans(se_ins_data[, sc_spots]))  # Matrix::rowSums
    sc_name <- paste(c, s, sep=".")
    colnames(sc_data) <- sc_name
...
# data_hm_norm <- (apply(se_ins_data_grouped, 2, cal_z_score))

fname <- "bulk_dea_insulin_cage_visium-cluster_heatmap"
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 6, height = 8);
...
p_hm <- pheatmap(data_hm_norm,  # se_ins_data_grouped, data_hm_norm
                 ...
                 breaks = breaksList,
                 color = viridis::viridis(length(breaksList)), # colors_ramp(length(breaksList)),  #colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))
                 # cluster_rows = F, 
                 border_color = "white"
                 )
dev.off()
...
```{r plot_visium_adipocyte_insulin_hm_score_all, fig.width=4, fig.height=8}
fname <- "bulk_dea_insulin_cage_visium-cluster_heatmap_log2ratio"
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 4, height = 8);

breaksList <- seq(-1, 1, by = 0.01)
pheatmap((log2(se_ins_data_score_ratio)),  # se_ins_data_grouped, data_hm_norm
         annotation_colors = my_colour,
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 6.5, height = 3);
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 5, height = 2.5);
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 12, height = 4);p1;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 12, height = 4);p2;dev.off()
```
...
se_ins_allc <- GetAssayData(se_ins, slot = "data")
```
...
for(s in s_names){
  for(c in c_ids){
      ...
      if (length(sc_spots)>1) {
        sc_data <- data.frame(Matrix::rowMeans(se_ins_allc[, sc_spots]))  # Matrix::rowSums Matrix::rowMeans  rowMeans_no0
...
for(s in unique(se_ins_metadata_allc$subject_id)){
  ...
  for(c in c_ids){
    ...
    if(up_sum_1==0 | up_sum_0==0 | down_sum_1==0 | down_sum_0==0){
      s_c_all$ratio <- 1
    }
...
cols_allc[c("C3", "C4")] <- colors_adi[c("Adipocyte 2", "Adipocyte 1")] # c("#F768A1", "#FBB4B9")
...
```{r plot_hm, fig.width=16, fig.height=8}
fname <- "bulk_dea_insulin_cage_visium-all_clusters_heatmap_log2ratio"
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 16, height = 8);
breaksList <- seq(-2, 2, by = 0.01)
pheatmap((log2(se_ins_data_score_allc_ratio)),  # se_ins_data_grouped, data_hm_norm
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 4.5, height = 12)
breaksList <- seq(-1, 1, by = 0.01)
...
```{r plot_visium_adipocyte_insulin_box_score_allc, fig.width=6, fig.height=2}
...
p1 <- ggplot(d_plot, aes(x=reorder(cluster, order) , y=up))+
  ...
  geom_hline(yintercept = mean(d_plot$up), linetype="dashed", color = "orange", alpha=0.8) +
  scale_color_manual(values = my_colour$m_value) +
...
p2 <- ggplot(d_plot, aes(x=reorder(cluster, order), y=down)) +
  ...
  geom_line(aes(group=subject_id), color = "grey80", alpha=0.6) +
  geom_point(size=2, aes(color=as.factor(m_value)), alpha=0.8) +
  geom_hline(yintercept = mean(d_plot$down), linetype="dashed", color = "orange", alpha=0.8) +
  scale_color_manual(values = my_colour$m_value) +
  ylim(min(d_plot[,1:2])-.02, max(d_plot[,1:2])+0.02) +
  labs(y="Expression log2-ratio") +
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 12, height = 4);
plot_grid(p1, p2, nrow=1)
dev.off()
```
