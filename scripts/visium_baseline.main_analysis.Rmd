...
p <- DimPlot(object = se_base, 
             reduction = "umap", dims = c(1,2), 
             group.by = "subject_alias",
             cols = colors_multi) + 
  NoAxes();
fname <- paste0(ANALYSIS_ID, ".umap_subject")
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 10, height = 5);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 4.8, height = 5);p2;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 10, height = 6);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 7, height = 11); p1;dev.off()
...
p2 <- DotPlot(se, features = rev(unique(top_dotp$gene)),
  ...
  # scale_colour_gradient2(low = colors_main[1], mid = "grey90", high = colors_main[2]) +
  # scale_colour_gradient2(low = "#3F4788FF", mid = "grey90", high = "#DCE318FF") +
  # scale_colour_gradient2(low = "#440154FF", mid = "grey90", high = "#FDE725FF") +
  scale_colour_gradient2(low = "#441153", mid = "grey90", high = "#3CB67B") +
  # scale_colour_gradientn(colours = colorRampPalette(rev(brewer.pal(11, "PuOr")))(12)[2:11]) +
  # scale_colour_gradient2(low = "#54AFC9", mid = "grey90", high = "#C95380") +
  # scale_colour_gradient2(low = "#54AFC9", mid = "grey90", high = "#c77c01") +  #c99654 "#fd8d3c"
  # scale_colour_gradient2(low = colors_multi[c(6)], mid = "grey90", high = colors_multi[c(2)]) + 
  theme(axis.title.x = element_blank(),
  # scale_color_viridis() +
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 12, height = 5.4); p2;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 8, height = 6);p2;dev.off()
