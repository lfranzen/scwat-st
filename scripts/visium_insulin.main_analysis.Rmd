---
title: "Visium data - 'Insulin' samples: Main processing and analysis using Seurat/STUtility"
...
Data processing and analysis of Visium data of subcutaneous white adipose tissue (scWAT) from 4 subjects with samples collected before and 2 h into a hyperinsulinemic-euglycemic clamp.  
...
metadata_selected <- metadata[!is.na(metadata$insulin_stim), ]  # all insulin samples (bef/aft)
...
c_anno <- data.frame(seurat_clusters = as.numeric(seq(1, 19)),
...
for( group in unique(c_anno$cluster_group) ){
  ...
  if(group == "Unknown"){
    # col_pal <- brewer.pal(gs_l+c_add+3, "BuPu")[(3+c_add):(gs_l+c_add+2)]
    col_pal <- color_func_purp(gs_l)
    c_anno[c_anno$cluster_group == group, "color_group"] <- "bupu"
  }
  print(col_pal)
...
Using a few known insulin responding genes, we can create a insulin response gene signature to look for in the Visium data.  
  
*"The Adipose Transcriptional Response to Insulin Is Determined by Obesity, Not Insulin Sensitivity"*, Rydén et al.  
  
  
```{r other_insulinsign}
genes_insulin <- c("THRSP", "BHLHE40", "PPP1R3B", "PIK3R3")  # TODO! Add more genes?
genes_insulin_intersect <- intersect(genes_insulin, rownames(se))
print(genes_insulin_intersect)

insulin_spot_sign <- colSums(GetAssayData(se, slot = "data", assay = "SCT")[genes_insulin_intersect, ])
print(summary(insulin_spot_sign))
se <- AddMetaData(se, insulin_spot_sign, col.name = "insulin_signature")
```


```{r other_insulinsign_df}
cut_ins <- median(se@meta.data[se@meta.data$insulin_signature>0, "insulin_signature"])

se@meta.data$insulin_cat <- "low"
se@meta.data[se@meta.data$insulin_signature > cut_ins, "insulin_cat"] <- "high"

df_ins_prop <- se@meta.data
df_ins_prop <- df_ins_prop %>%
  group_by(condition, subject_alias, m_value, insulin_stim, insulin_cat) %>%
  dplyr::count()

df_ins_prop$pct <- 0
for(s in unique(df_ins_prop$subject_alias)) {
  sum_spots <- sum(df_ins_prop[df_ins_prop$subject_alias %in% s, "n"])
  df_ins_prop[df_ins_prop$subject_alias %in% s, "pct"] <- (round(df_ins_prop[df_ins_prop$subject_alias %in% s, "n"] / sum_spots, digits = 3))*100
}

df_ins_prop$cut <- cut_ins
df_ins_prop
```
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 3, height = 3);p;dev.off()
```
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 32, height = 18);p;dev.off()
...
p1 <- DimPlot(object = se, dims = c(1,2), reduction = "umap", group.by = "subject_alias", 
              pt.size = pt_size, cols = colors_multi) + NoAxes() #+ DarkTheme()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 10, height = 8.5);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 16, height = 5);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 10, height = 2);p;dev.off()
```
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 6, height = 5);p2;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 14, height = 6);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 6, height = 2.5);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 14, height = 7);p;dev.off()
...
```{r markers_clusterdea_dotplot_a, fig.width=7, fig.height=12}
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 7, height = 12); p1;dev.off()
tiff(filename = file.path(DIR_FIG, paste0(fname, ".tiff")), units = "cm", width = 7*2.5, height = 12*2.5, res = 600); p1; dev.off()
ggsave(filename = file.path(DIR_FIG, paste0(fname, ".eps")), plot = p1, width = 7, height = 12)
```
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 14, height = 5.4); p2;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 22, height = 16);p;dev.off()
...
fname <- paste0(ANALYSIS_ID, ".baseline.markers_pea_GO_a")
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 8, height = 7);p1;dev.off()
...
```{r markers_pea_GO_dotplot_b, fig.width=10, fig.height=6}
...
fname <- paste0(ANALYSIS_ID, ".baseline.markers_pea_GO_b")
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 10, height = 6);p2;dev.off()
tiff(filename = file.path(DIR_FIG, paste0(fname, ".tiff")), units = "cm", width = 10*2.5, height = 6*2.5, res = 600); p2; dev.off()
ggsave(filename = file.path(DIR_FIG, paste0(fname, ".eps")), plot = p2, width = 10, height = 6)
```
...
fname <- paste0(ANALYSIS_ID, ".baseline.markers_pea_RA_a")
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 8, height = 7);p1;dev.off()
tiff(filename = file.path(DIR_FIG, paste0(fname, ".tiff")), units = "cm", width = 8*2.5, height = 7*2.5, res = 600); p1; dev.off()
ggsave(filename = file.path(DIR_FIG, paste0(fname, ".eps")), plot = p1, width = 8, height = 7)
```
...
fname <- paste0(ANALYSIS_ID, ".baseline.markers_pea_RA_b")
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 10, height = 6);p2;dev.off()
tiff(filename = file.path(DIR_FIG, paste0(fname, ".tiff")), units = "cm", width = 10*2.5, height = 6*2.5, res = 600); p2; dev.off()
ggsave(filename = file.path(DIR_FIG, paste0(fname, ".eps")), plot = p2, width = 10, height = 6)
```
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 8, height = 3.5);p;dev.off()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 12, height = 3.5);p;dev.off()
...
```{r other_insulinsign_bar, fig.width=6, fig.height=4}
p <- ggplot(subset(df_ins_prop, insulin_cat == "high"), aes(x = subject_alias, y = pct, fill = as.factor(insulin_stim) )) +
  geom_bar(position="dodge", stat="identity", width=0.7) + 
  scale_fill_manual(values = colors_insulin) +
  labs(x="", fill="", y = "Proportion of total spots (%)", title="Spots with high insulin signature score") +
  theme_minimal() +
...
fname <- paste0(ANALYSIS_ID, ".other_insulinsign_bar")
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 6, height = 4);p;dev.off()
tiff(filename = file.path(DIR_FIG, paste0(fname, ".tiff")), units = "cm", width = 6*2.5, height = 4*2.5, res = 600); p; dev.off()
```
