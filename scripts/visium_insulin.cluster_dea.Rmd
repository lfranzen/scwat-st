...
clustercond_markers_df <- do.call(rbind.data.frame, clustercond_markers)
clustercond_markers_df <- clustercond_markers_df[clustercond_markers_df$p_val_adj < 0.05, ]
...
for (c in c_include) {
  for (s in s_names ) {
    ...
    if (nrow(subset(se[[]], sample_name == s & seurat_clusters == c)) > 0) {
