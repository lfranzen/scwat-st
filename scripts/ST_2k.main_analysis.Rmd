...
colors_main_paired <- c("#A5D7E5", "#4773B7", "#FEC567", "#F15C26")
colors_tri <- c(colors_main, "#db7f9b")
colors_quad <- c(colors_tri, "grey60")
# colors_scale <- c("black", "darkblue", "cyan", "yellow", "red", "darkred")
# colors_rbow <- colorspace::rainbow_hcl(20)
...
colors_multi <- c("#a5d7e5",
                  ...
                  "#fb7272",
                  "grey50")
colors_clusters <- c("grey85", colors_multi, "grey30", "grey50", "grey70")
...
```{r sct}
n_var_feat <- 7000
se <- SCTransform(se, variable.features.n = n_var_feat, verbose = T)
```
...
cluster_prop <- subset(se)@meta.data
cluster_prop <- cluster_prop %>% 
  group_by(subject_alias, seurat_clusters, condition) %>%
  dplyr::count()
cluster_prop <- as.data.frame(cluster_prop)

subject_spot_count <- aggregate(cluster_prop$n, by=list(subject = cluster_prop$subject_alias), FUN=sum)
colnames(subject_spot_count)[2] <- "total_spots"

cluster_prop <- merge(cluster_prop, subject_spot_count, by=c("subject"))
cluster_prop$cluster_pct_subject <- round((cluster_prop2$n / cluster_prop2$total_spots), digits = 3)*100
...
df_M1_prop <- df_M1_prop %>%
  group_by(condition, subject, ATM_M1_cat) %>%
  dplyr::count()
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 14, height = 20);p1;p2;p3;dev.off()
``` 
  
  
### UMAP  

```{r dimred_umap_plot, fig.width=12, fig.height=10}
pt_size <- 0.5
...
```{r dimred_umap_plot_genes, fig.width=12, fig.height=10}
pt_size <- 0.25
...
plot_grid(plot_gene_umap(se, "SAA1"),
          ...
          plot_gene_umap(se, "RNASE1"),
          ncol = 3)
```
  
  
```{r dimred_umap_ica_plot, fig.width=12, fig.height=5}
pt_size <- 0.5
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 12, height = 5);p;dev.off()
tiff(filename = file.path(DIR_FIG, paste0(fname, ".tiff")), units = "cm", width = 12, height = 5, res = 600); p; dev.off()
```
...
```{r clustering_umap, fig.width=6, fig.height=6}
p <- DimPlot(object = se, 
...
pdf(file = file.path(DIR_FIG, paste0(fname, ".pdf")), width = 6, height = 6);p;dev.off()
tiff(filename = file.path(DIR_FIG, paste0(fname, ".tiff")), units = "cm", width = 14, height = 14, res = 600); p; dev.off()
```
  
  
### Spot proportions  
  
```{r clustering_countspots, fig.width=14, fig.height=6}
d_plot <- cluster_prop
...
p2 <- DotPlot(se, features = unique(top_dotp$gene),
             col.min = -2, col.max = 2,
             group.by = "seurat_clusters",
             scale = T,
             cols = c("grey90", "red")) +
...
```{r markers_clusterdea_heatmap, fig.width=6, fig.height=8}
...
pdf(file = file.path(DIR_FIG, "markers_clusterdea_heatmap.pdf"), width = 6, height = 8);p;dev.off()
tiff(filename = file.path(DIR_FIG, "cmarkers_clusterdea_heatmap.tiff"), units = "cm", width = 14, height = 18, res = 600); p; dev.off()
```
